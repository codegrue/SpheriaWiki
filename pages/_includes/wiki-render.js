...
function bindTabs() {
  const buttons = document.querySelectorAll(".sub-menu-btn");
...
function characterCard(c, border) {
  ...
  const fields = [
    ...
    c.legs != null
      ? `<span class="pill">${escapeHtml(String(c.legs))} Legs</span>`
      : c.age != null
        ? `<span class="pill">${escapeHtml(String(c.age))} years old</span>`
        : "",
...
function renderObjects(objects) {
  return `<div class="info-grid">${objects.map(objectCard).join("")}</div>`;
}
...
function renderWorldPhysics(world) {
  ...
  const html = Object.keys(physics)
    .map((key) => {
      const node =
        typeof physics[key] === "object"
          ? physics[key]
          : { description: physics[key] };
      const title = node.use
        ? `${titleFromKey(key)} - ${node.use}`
        : titleFromKey(key);
      const effectPill = node.effect
        ? `<div class="pill-row" style="margin-top:6px"><span class="pill">${escapeHtml(node.effect)}</span></div>`
        : "";
      const notes = node.notes
        ? `<div class="source-color-trigger">Notes: ${escapeHtml(node.notes)}</div>`
        : "";
      return `
        <div class="info-card">
...
async function loadFooter(containerId = "footer-container") {
  ...
  if (!container) return;
  try {
    const response = await fetch("/pages/_includes/footer.html", {
      cache: "default",
    });
...
async function initCharacterTabs() {
  try {
    ...
    if (humansContainer) humansContainer.innerHTML = renderHumans(data.characters);
    if (polyansContainer) polyansContainer.innerHTML = renderPolyanCharacters(data.characters);
    bindTabs();
  } catch (err) {
    const el = document.getElementById("humans-content");
    if (el) {
      el.classList.add("error");
      el.innerHTML =
        "Could not load Spheria_Wiki.json. Run a local web server (npm run dev) and refresh.";
...
async function initObjectTabs() {
  try {
    ...
    const el = document.getElementById("earth-content");
    if (el) {
      el.classList.add("error");
      el.innerHTML =
        "Could not load Spheria_Wiki.json. Run a local web server (npm run dev) and refresh.";
...
async function initWorldTabs() {
  try {
    ...
    const el = document.getElementById("geography-content");
    if (el) {
      el.classList.add("error");
      el.innerHTML =
        "Could not load Spheria_Wiki.json. Run a local web server (npm run dev) and refresh.";
