...
function renderOverview(data){
  const stats = [
    ...
    { label: "World Facts", value: Object.keys(data.world || {}).length, border: "#0ea5e9", color: "#38bdf8" },
    { label: "Polyan Lore", value: Object.keys(data.polyans || {}).length, border: "#8b5cf6", color: "#c4b5fd" },
    { label: "Mythos", value: Object.keys(data.mythos || {}).length, border: "#f59e0b", color: "#fbbf24" },
    { label: "Flora/Fauna", value: Object.keys(data.flora_fauna || {}).length, border: "#22c55e", color: "#4ade80" }
  ];
...
function renderLegDots(legs){
  ...
  const dots = Array.from({ length: 7 }, (_, i) => `<span class="leg-dot${i < count ? " active" : ""}"></span>`).join("");
...
function renderWorld(world){
  ...
  const physicsHtml = Object.keys(physics).map(key => `
  ...
  const sourceColors = (world.source_colors || []).map(item => `
...
function renderPolyans(polyans){
  if(!polyans) return `<div class="status-msg error">Polyan data is missing.</div>`;

  const casteSystem = (polyans.caste_system || []).map(item => `
...
function renderMythos(mythos){
  ...
  return `
    ...
    <div class="info-grid">
      ${Object.keys(lumen).map(key => `<div class="info-card" style="border-top-color:#60a5fa"><div class="info-title icon-title"><span class="icon">📜</span>${escapeHtml(titleFromKey(key))}</div><div class="info-body">${escapeHtml(lumen[key])}</div></div>`).join("")}
    </div>
...
function renderFloraFauna(data){
  if(!data) return `<div class="status-msg error">Flora/Fauna data is missing.</div>`;

  const fauna = (data.fauna || []).map(item => `
  ...
  const flora = (data.flora || []).map(item => `
  ...
  const crystals = (data.crystals || []).map(item => `
...
async function initPage(pageKey, containerId){
  try{
    ...
    else if(pageKey === "polyans") container.innerHTML = renderPolyans(data.polyans);
    else if(pageKey === "mythos") container.innerHTML = renderMythos(data.mythos);
    else if(pageKey === "flora-fauna") container.innerHTML = renderFloraFauna(data.flora_fauna);
